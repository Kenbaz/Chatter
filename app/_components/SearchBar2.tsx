...
const SearchBar: FC = () => {
  ...
  return (
    <div className="search-bar-container outline-none border relative w-full rounded-lg dark:border-secondary z-50">
      ...
      {showDropdown && (
        <div
          ref={dropdownRef}
          className="search-results-dropdown absolute w-full border border-t-0 rounded-t-sm dark:border-customGray1  rounded-b-lg dark:bg-customGray1 bg-customWhite3 p-1"
        >
          {results.length > 0 ? (
            results.map((post) => (
              <Link key={post.id} href={`/post/${post.id}`}>
                <div className="search-result-item mb-2 dark:hover:bg-customGray hover:bg-customWhite2 p-1 px-2 rounded-lg">
                  <small
